...
const legacyHandlers = {
	...
	createRecording: function createRecording(recording, callback) {
	...
	getRecordings: function getRecordings(options, callback) {
	...
	getRecording: function getRecording(recordingId, callback) {
	...
	updateRecording: function updateRecording(recording, callback) {
		validate.recording(recording);
	...
	updateRecordingFolder: function updateRecordingFolder(folder, callback) {
	...
	updateCms: function updateCms(recordingId, callback) {
		if (typeof recordingId !== 'string' || recordingId === '') {
			throw new Error('Missing recordingId!');
		}
		if (typeof callback !== 'function') {
	...
	deleteRecording: function deleteRecording(recordingId, callback) {
		if (typeof recordingId === 'number') {
			recordingId = recordingId + '';
		}
		if (typeof recordingId !== 'string' || recordingId === '') {
			...
			request(
				{
					method: 'GET',
					uri:
						self._baseUri + endpoints.recording + recordingId + '/transcript',
					headers: generateHeaders(self._token),
					json: true
				},
				function requestCallback(err, response, body) {
			...
			request(
				{
					method: 'POST',
					url:
						self._baseUri + endpoints.job + jobId + '/task/' + taskId + '/poll',
					headers: generateHeaders(self._token),
					json: data
				},
				function requestCallback(err, response, body) {
