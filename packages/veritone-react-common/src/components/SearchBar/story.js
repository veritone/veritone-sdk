...
export default class SampleSearchBar extends React.Component {
  ...
  addOrModifySearchParameter = ( parameter ) => {
    const index = this.state.searchParameters.findIndex( searchParameter => searchParameter.id === parameter.id );
    if ( index !== -1 ) {

      const newSearchParameters = update(this.state.searchParameters, {$splice: [[index, 1, parameter]]} );
  ...
  removeSearchParameter = ( id ) => {
    this.setState(prevState => ({ searchParameters: prevState.searchParameters.filter(x => x.id !== id) }))
  ...
  searchQueryGenerator = () => {
    ...
    this.state.searchParameters.map( (searchParameter) => {
      searchCategoryFilters.conditions.push(
        engineCategoryMapping[searchParameter.engineId].generateCondition(searchParameter)
      );
    });
  ...
  extendEngineCategories = ( engineCategories ) => {
    const engineCategoriesWithFunctions = engineCategories.map( engineCategory => {
      if(engineCategory.id in engineCategoryMapping) {
        return {...engineCategory, ...engineCategoryMapping[engineCategory.id] }
      }
    } );
  ...
  render() {
    return (
      <div style={{ height: '100%', width: '100%', background: appBarColor, padding: '0px', display: 'flex', alignItems: 'center' }}>
        <div style={{ margin: '0 1em 0 1em', padding: 0 }}>
            <h3>Logo</h3>
        </div>
        <SearchBarContainer color={appBarColor} enabledEngineCategories={ this.extendEngineCategories(enabledEngineCategories) } searchParameters={ this.state.searchParameters } addOrModifySearchParameter={ this.addOrModifySearchParameter } removeSearchParameter={ this.removeSearchParameter } />
        <button onClick={this.searchQueryGenerator}>
          Search
        </button>
      </div>
    )
