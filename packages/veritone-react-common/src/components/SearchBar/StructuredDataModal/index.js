...
const getStructuredDataLabel = modalState => {
  ...
  const getAbbreviation = (modalState) => {
    if (modalState.operator === 'range') {
      return `${modalState.field.split('.').slice(-1)[0]} ${OPERATOR_ABRV[modalState.operator]} (${modalState.value1},${modalState.value2})`
      ...
      return  `${modalState.field.split('.').slice(-1)[0]} ${OPERATOR_ABRV[modalState.operator]} ${modalState.value1}`
    }
  }
  return {
