...
class SearchBarContainer extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      (!prevState.openModal || !prevState.openModal.modalId) &&
      this.state.openModal &&
      this.state.openModal.modalId
    ) {
      this.windowResizeListener = fromEvent(window, 'resize')
        .map(x => x.currentTarget.outerWidth)
        .debounceTime(150)
      ...
      prevState.openModal &&
      prevState.openModal.modalId &&
      (!this.state.openModal || !this.state.openModal.modalId)
    ) {
  ...
  handleResetAdvanced = () => {
    ...
    this.setState(state => ({
      ...state,
      advancedEnableIds: state.advancedEnableIds.filter(
        item => item !== modalId
      ),
      advancedOptions: {
        ...state.advancedOptions,
  ...
  get getBadgeLength() {
    ...
    if (
      !isEmpty(boundingPoly) &&
      !isEmpty(range) &&
      !isEqual(sortBy(range), sortBy([0, 100]))
    ) {
      return 2;
    }
    if (
    ...
    if (
      !isEmpty(boundingPoly) &&
      (isEmpty(range) || isEqual(sortBy(range), sortBy([0, 100])))
    ) {
      return 1;
    }
    return null;
  ...
  handleGroupingKeyPress = event => {
    ...
    if (
      event.code === 'Escape' &&
      this.state.highlightedPills.length > 0 &&
      ...
      event.code === 'KeyG' &&
      event.shiftKey &&
      this.state.highlightedPills.length > 1
    ) {
      event.preventDefault();
      this.toggleGrouping();
    }
  ...
  toggleGrouping = () => {
    ...
    if (
      before &&
      before.conditionType === 'group' &&
      before.value === '(' &&
      after &&
      after.conditionType === 'group' &&
      after.value === ')'
    ) {
  ...
  togglePill = (searchParameterId, searchParameters) => {
    if (this.state.highlightedPills.length > 0) {
      ...
      const clickTargetShouldNotClearHighlightedPills = e => {
        const clickedOnHighlightedPill =
          e.path.find(
            y =>
              y.attributes &&
              y.attributes.getNamedItem('data-searchparameterid') &&
              this.state.highlightedPills.indexOf(
        ...
        const clickedOnDeleteFromMenu = e.path.find(
          y =>
            ...
            y.attributes.getNamedItem('data-preservehighlight').value === 'true'
        );
        return (
          e.shiftKey === true ||
          clickedOnHighlightedPill ||
          clickedOnDeleteFromMenu
        );
      };
      ...
      this.unselectMouseClick = fromEvent(document, 'mousedown')
        .takeWhile(clickTargetShouldNotClearHighlightedPills)
        .subscribe(null, null, () => {
          this.setState({ highlightedPills: [] });
        });
  ...
  simplifySearchParameters = searchParameters => {
    const reduced = searchParameters.reduce(
      (accu, searchParameter) => {
        ...
        if (
          ...
          simplified[simplified.length - 2].value === '(' &&
          simplified[simplified.length - 1] &&
          simplified[simplified.length - 1].value !== '(' &&
          simplified[simplified.length - 1].value !== ')' &&
    ...
    while (
      ...
      simplifiedParameters[simplifiedParameters.length - 1].value === ')'
    ) {
      extraneousGroups.push(simplifiedParameters[0]);
      extraneousGroups.push(
        simplifiedParameters[simplifiedParameters.length - 1]
      );
  ...
  simpleRemovePill = (searchParameterId, searchParameters) => {
    const paramsContentTypes = searchParameters.map(
      param => param.conditionType
    );
    const { advancedEnableIds, advancedOptions } = this.state;
    advancedEnableIds.forEach(id => {
      const newAdvancedEnableIds = advancedEnableIds.filter(
        item => item !== id
      );
    ...
    if (
      ...
      (index === 0 && this.numberOfPills(searchParameters) > 1)
    ) {
      pillsToRemove.push(searchParameterId);
      pillsToRemove.push(searchParameters[index + 1].id);
      newSearchParameters = searchParameters.filter(
        x =>
          x.id !== searchParameterId && x.id !== searchParameters[index + 1].id
      );
      ...
      newSearchParameters = searchParameters.filter(
        x => x.id !== searchParameterId && x.id !== lastJoiningParameter.id
      );
    } else {
  ...
  numberOfPills = searchParameters =>
    searchParameters.reduce((accu, searchParameter) => {
      if (
        searchParameter.conditionType !== 'join' &&
  ...
  addNewSearchParameter = (parameter, engineId) => {
    ...
    if (this.numberOfPills(this.props.searchParameters) > 0) {
      const lastJoiningOperator = this.getLastJoiningOperator(
  ...
  handleMenuOpen = (target, searchParameter) => {
    ...
    if (searchParameter.conditionType === 'join') {
      ...
      const showGroupOptions =
        this.state.highlightedPills.length > 1 &&
        this.state.highlightedPills.includes(searchParameter.id);
      if (showGroupOptions) {
        menuOptions = [
          {
  ...
  openMenuExtraActions = evt => {
    let customMenuActions =
      (this.props.menuActions &&
        this.props.menuActions.map(x => ({
          label: x.label,
          onClick: () => {
    ...
    if (customMenuActions && customMenuActions.length > 0) {
      customMenuActions = [...customMenuActions, { divider: true }];
    }
  ...
  menuRemoveHighlightedPills = () => {
    ...
    this.state.highlightedPills &&
      this.state.highlightedPills.forEach(highlightedPill => {
        simplifiedParameters = this.simpleRemovePill(
          highlightedPill,
          simplifiedParameters
        );
      });
  ...
  menuEditPill = () => {
    const selectedPill = this.props.searchParameters.find(
  ...
  addOrEditModal = () => {
    if (this.state.selectedPill) {
      ...
      if (this.state.insertDirection) {
        ...
        const insertAt =
          this.state.insertDirection === 'left'
            ? selectedPillIndex
            : selectedPillIndex + 1;
        ...
        const newParams =
          (selectedParamConditionType === 'join' &&
            this.state.insertDirection === 'left') ||
          (selectedParamConditionType !== 'join' &&
            this.state.insertDirection === 'right')
  ...
  render() {
    ...
    const libraryIds =
      this.props.libraries && this.props.libraries.map(library => library.id);
    const selectedPill = this.props.searchParameters.find(
      x => x.id === this.state.selectedPill
    );
    const { isAdvancedSearchEnabled } = this.props;
    const horizontalAnchorPosition =
      this.state.menuAnchorEl && this.state.menuAnchorEl.type === 'button'
        ? { horizontal: 'right' }
        : { horizontal: 'left' };
    return (
      <div
        ...
        style={{ width: '100%', overflowY: 'hidden', borderRadius: '8px' }}
        data-veritone-component={`search_bar_${this._id}`}
      >
        <SearchBar
        ...
        >
          {this.state.menuOptions &&
            this.state.menuOptions.map(menuOption =>
              menuOption.divider ? (
                <Divider key={'menu_divider'} />
              ) : (
                <MenuItem
                  key={menuOption.label}
                  data-veritone-element={`${menuOption.label
                    .toLowerCase()
                    .split(' ')
                    .join('_')}`}
                  onClick={menuOption.onClick}
                  data-preservehighlight={menuOption.preserveHighlight}
                >
                  {menuOption.label}
                </MenuItem>
              )
          ...
          >
            ...
            >
              <CardHeader
                    ...
                    {this.props.enabledEngineCategories &&
                      this.props.enabledEngineCategories.map(engineCategory => (
                        <EngineCategoryButton
                          ...
                          backgroundColor={
                            engineCategory.id === this.state.openModal.modalId
                              ? this.props.color
                              : undefined
                          }
                          color={
                ...
                title={
                  <span
                    data-veritone-element={`search-category-label-${openModal.dataTag}`}
                  >
                    {openModal.title}
                  </span>
                }
              ...
              />
              <CardContent
                style={{ margin: '0.5em', paddingTop: '0', paddingBottom: '0' }}
              >
                {Modal ? (
                  <Modal
                    ...
                    presetSDOSchema={
                      this.state.openModal.modalId === 'sdo-search-id'
                        ? this.props.presetSDOSchema
                        : undefined
                    }
                    presetSDOAttribute={
                      this.state.openModal.modalId === 'sdo-search-id'
                        ? this.props.presetSDOAttribute
                        : undefined
                    }
                    sourceFilters={
                      this.state.openModal.modalId === 'sdo-search-id'
              ...
              >
                ...
                >
                  {this.state.selectedPill && !this.state.insertDirection
                    ? selectedPill.conditionType === openModal.id &&
                      this.state.openModal.modalState !== undefined
                      ? 'Save'
                      : 'Replace'
                    : 'Add'}
                </Button>
