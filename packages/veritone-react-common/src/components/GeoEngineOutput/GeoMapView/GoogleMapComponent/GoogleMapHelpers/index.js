...
export function genMarker (position, icon=null, map=null, title='', draggable=false) {
  if (!window.google || !window.google.maps) {
    return null;
  }
...
export function genOriginMarker(position, opacity=1, map, title='', draggable=false) {
  if (!window.google || !window.google.maps) {
    return null;
  }
...
export function genDestinationMarker(position, opacity=1, map, title='', draggable=false) {
  if (!window.google || !window.google.maps) {
    return null;
  }
...
export function drawDirection (startPoint, endPoint, map, midPoints=[], travelMode=travelModes.FLYING) {
  if (!window.google || !window.google.maps) {
    return null;
  }
    ...
    if (travelMode === travelModes.FLYING) {
      ...
      if (midPoints && midPoints.length > 0) {
        midPoints.forEach((point, index) => {
          if ((index === 0 && point === startPoint) || (index === midPoints.length - 1 && point === endPoint)) {
            return;
          }
          ...
          if (!waypointLocation && !(waypointLat == null) && !(waypointLng == null)) {
      ...
      while (waypoints.length > 0) {
        ...
        const destination = (waypoints.length > 0) ? waypoints.splice(0, 1)[0] : endPoint;
        ...
        directionsService.route(request, (result, status) => {
          if (status == 'OK') {
            result.routes && result.routes.forEach(route => {
              route.overview_path && route.overview_path.forEach(entry => {
                path.push(entry)
              });
            });
            ...
            if (numWaypointChunks === 0) {
              ...
              (firstEntry.lat !== startPoint.lat || firstEntry.lng !== startPoint.lng) && path.splice(0, 0, startPoint);
              (lastEntry.at !== endPoint.lat || lastEntry.lng !== endPoint.lng) && path.push(endPoint);
...
export function findDistanceRatio (lat1, lng1, lat2, lng2) {
  ...
  const haversine = Math.pow(Math.sin((lat2Rad - lat1Rad) / 2), 2) + 
