...
import { string, func, object, any } from 'prop-types';
...
export default class SelectSchemas extends React.Component {
  ...
  loadMoreData = async schemaName => {
    if (
      ...
      const offset =
        schemaName === this.state.lastSearch ? this.state.offset : 0;
      let data = schemaName === this.state.lastSearch ? this.state.data : [];

      const results = await fetchSchemas({
        api: this.props.api,
      ...
      data = data.concat(
        this.reduceData(get(results, 'data.dataRegistries.records'))
      );

      const count = get(results, 'data.dataRegistries.count');
  ...
  render() {
    return (
      <InfiniteSelect
        ...
        defaultSelected={
          this.getSelected().name === this.state.firstOption.name
        }
        onSelect={this.onSelect}
        resetSelect={this.resetSelect}
        display={schema => <Schema {...schema} />}
      />
    );
  }
}
