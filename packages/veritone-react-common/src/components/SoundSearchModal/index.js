...
// Sound Autocomplete config:
const soundConfig = {
  identifierType: 'sound',
  searchLibrary: false,
  searchEntity: false,
  ...
  customFields: ['audiodetection-audiosecurity-hpe.series.found'],  // For object/logo/tags
...
export default class SoundSearchModal extends React.Component {
  static defaultProps = {
    ...
    applyFilter: value => console.log('Search sounds by entityId', value),
...
export const SoundSearchForm = ( { cancel, applyFilter, onChange, onKeyPress, modalState, selectResult } ) => {
  return (
  <div>
    <DialogTitle>Search by Sound</DialogTitle>
    <DialogContent style={{ width: '500px', margin: 'none' }}>
      <SearchAutocompleteContainer
        id="sound_autocomplete_container"
        onChange={ onChange }
...
const SoundConditionGenerator = modalState => {
  // TODO: implement translator
  return {};
};

const SoundDisplay = modalState => {
  return {
