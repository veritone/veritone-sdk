...
storiesOf('Share Components', module).add('Pill Button', () => {
  return (
...
class DynamicScrollContainer extends Component {
  ...
  genContents (startValue, stopValue, minStepSize = 20, maxStepSize = 50) {
    let contents = [];
    let stepSize = Math.round(Math.random() * (maxStepSize - minStepSize)) + minStepSize;
    for (let value = startValue; value < stopValue; value = value + stepSize) {
      contents.push({
        start: value,
        stop: value + stepSize,
        value: (<DummyComponent key={'key-dummy'+value+'-'+value + stepSize}/>)
      });
    }
...
class DummyComponent extends Component {
  render () {
    ...
    return (
      <div className={styles.dummyContentBox} style={{height: randomHeight, background: randomColor}}>Dummy Content Box</div>
    )
  };
}
