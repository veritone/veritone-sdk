...
export default class DynamicContentScroll extends Component {
  ...
  scrollTo = (value) => {
    if (value >= 0) {
      ...
      for (let anchorIndex = 0, numAnchors = anchors.length; anchorIndex < numAnchors; anchorIndex++) {
  ...
  requestMoreContents = () => {
    if (this.props.onScroll) {
      ...
      for (let fillerIndex = 0, numFillers = fillers.length; fillerIndex < numFillers; fillerIndex++) {
        let filler = fillers[fillerIndex];
        let fillerBox = filler.getBoundingClientRect();

        if (fillerBox.top < containerBox.bottom) {
          if (fillerBox.top > containerBox.top || fillerBox.bottom > containerBox.top) {
      ...
      (finalStart !== undefined && finalStop !== undefined) && this.props.onScroll({start: finalStart, stop: finalStop});
    }
  }
  ...
  renderFillers (start, stop) {
    ...
    while (currentStart + neglectableSize < stop) {
      if (currentStart + segmentSize < stop) {
        ...
        fillers.push(<div key={'key-'+currentStart+'-'+nextStart} name={'filler'} anchor={'time-anchor'} start={currentStart} stop={nextStart} className={styles.fillerSegment} style={{height:segmentHeight}}/>);
        currentStart = nextStart;
      } else {
        fillers.push(<div key={'key-'+currentStart+'-'+stop} name={'filler'} anchor={'time-anchor'} start={currentStart} stop={stop} className={styles.fillerSegment} style={{height:segmentHeight}}/>);
        break;
      }
    }
  ...
  renderContent () {
    ...
    // Sort Contents
    contents ? contents = sortBy(this.props.contents, 'start', 'stop') : contents = [];    
    ...
    for (let contentIndex = 0; contentIndex < numContents; contentIndex++) {
      ...
      // Add Content
      renderItems.push(<span key={'anchor-key-' + content.start + '-' + content.stop} anchor={'time-anchor'} start={content.start} stop={content.stop} />); // add anchor for scroll to
      renderItems.push(content.value);
      prevStopPoint = content.stop;

      // Add fillers after content if needed
      if (contentIndex === numContents - 1 && prevStopPoint + neglectableSize < totalSize) {
  ...
  render () {
    ...
    return (
        <div ref={this.setContainerRef} className={classNames(styles.dynamicVScroll, this.props.className)} onScroll={this.handleScrolling}>
          { this.renderContent() }
        </div>
    );
  }
