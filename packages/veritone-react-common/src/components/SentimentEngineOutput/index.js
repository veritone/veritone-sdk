...
export default class SentimentEngineOutput extends Component {
  ...
  extractPropsData = () => {
    ...
    let xTicks = [];
    for (let xTickValue = 0; xTickValue <= totalTime; xTickValue = xTickValue + timeTickIntervalMs) {
        ...
        <div className={styles.yAxis}>
          <ResponsiveContainer width="100%" height="100%" >
            <AreaChart data={chartData} margin={{top: 10, right: 30, left: 0, bottom: 40}}>
              ...
              <YAxis dataKey="sentiment" domain={yDomain} ticks={yTicks} tickFormatter={this.formatYValues} type="number"/>
            </AreaChart>
          </ResponsiveContainer>
        </div>

        {/* Draw Chart Content Container */}
        <div className={styles.content} onScroll={this.handleTimeScroll} ref={this.setScrollRef}>

          {/* Draw Scrollable Chart Content */}
          <ResponsiveContainer width={(100 * scaleX).toString() + '%'} height="100%">
            <AreaChart data={chartData} margin={{top: 10, right: 0, left: 0, bottom: 40}} onClick={this.handleTimeClick} >
              <XAxis interval="preserveStartEnd" dataKey="stopTimeMs" domain={xDomain} ticks={xTicks} tickFormatter={this.formatXValues} type="number"/>
              <YAxis hide dataKey="sentiment" domain={yDomain} ticks={yTicks} tickFormatter={this.formatYValues}/>
              <CartesianGrid />
              ...
              <Area type="monotone" dataKey='sentiment' stroke='url(#splitColor)' fill='url(#splitColor)'/>
