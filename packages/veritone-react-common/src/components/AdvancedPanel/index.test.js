...
describe('Advanced Panel', () => {
    ...
    it('Advanced Search: Should render with the default value filled in', () => {
        ...
        expect(wrapper.find('.only-return-text').first().text()).toEqual(`Only return search results for this ${searchByTag} if they appear in a defined region.`);
        expect(wrapper.find('.only-return-text').last().text()).toEqual(`Search by the percentage of confidence of this ${searchByTag}.`);
