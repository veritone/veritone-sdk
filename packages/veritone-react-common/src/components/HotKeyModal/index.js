...
export default class HotKeyModal extends React.Component {
  ...
  render() {
    ...
    const { onClose, title, open, hotKeyCategories } = this.props;
                ...
                hotKeyCategory.commands.map(command => {
                  const hotkeyList = command.hotkeys
                    .filter(hotkey => {
                      ...
                      const isCompatible = platforms.find(
                        p => p.toLowerCase() === platformPrefix.toLowerCase()
                      );
                      return isCompatible;
                    })
                    .map(hotkey => {
                  ...
                  return (
                    <Grid
                      key={`hotkey-${hotKeyCategory.label}-command-${
                        command.label
                      }`}
                      spacing={24}
                      item
                      ...
                      alignItems="center"
                    >
                      <Grid item xs={3} className={styles.shortKeyTitle}>
                        {command.label}
                      </Grid>
