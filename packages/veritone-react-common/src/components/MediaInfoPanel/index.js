...
class MediaInfoPanel extends Component {
  ...
  onSaveMetadata = (metadataToSave) => {
    ...
    if (metadataToSave.veritoneFile && metadataToSave.veritoneFile.filename) {
      detailsParams.push(`veritoneFile: { filename: "${metadataToSave.veritoneFile.filename}" }`);
    }
    if (metadataToSave.veritoneCustom && metadataToSave.veritoneCustom.source) {
      detailsParams.push(`veritoneCustom: { source: "${metadataToSave.veritoneCustom.source}" }`);
    }
    if (metadataToSave.veritoneProgram && (metadataToSave.veritoneProgram.programLiveImage || metadataToSave.veritoneProgram.programImage)) {
      let programData = '';
      if (metadataToSave.veritoneProgram.programLiveImage) {
        programData += `programLiveImage: "${metadataToSave.veritoneProgram.programLiveImage}"`;
      }
      if (metadataToSave.veritoneProgram.programImage) {
        programData += ` programImage: "${metadataToSave.veritoneProgram.programImage}"`;
      }
    ...
    const contentElement = (
      <div className={styles.mediaInfoPanel}>
          ...
          <Paper className={styles.infoPanelContent}>
            ...
            {this.props.tdo.details.tags &&
              <div className={styles.infoField}>
                ...
                <div className={styles.infoFieldData}>{this.props.tdo.details.tags.map(tag => tag.value).join(', ')}</div>
              </div>}
        ...
        {this.props.tdo && this.props.tdo.details  &&
          <EditMetadataDialog isOpen={this.state.isOpenEditMetadata}
                              metadata={this.props.tdo.details}
                              onClose={this.toggleIsOpenEditMetadata}
                              onSave={this.onSaveMetadata} />}
        {this.props.tdo && this.props.tdo.details  &&
          <EditTagsDialog isOpen={this.state.isOpenEditTags}
