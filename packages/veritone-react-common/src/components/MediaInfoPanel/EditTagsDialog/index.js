...
class EditTagsDialog extends Component {
  ...
  hasPendingChanges = () => {
    return JSON.stringify(this.state.tags) !== JSON.stringify(this.props.tags) ||
      (this.state.newTagTextAreaVisible && this.state.newTagValue && this.state.newTagValue.length);
  };
          ...
          <DialogContent>
            ...
            <div className={styles.tagsSection}>
              {this.state.tags && this.state.tags.map(tag => (
                <TagPill className={styles.tagPill} key={tag.value} text={tag.value} onRemove={this.onRemoveTag} />
              ))}
              {!this.state.newTagTextAreaVisible &&
                <Button onClick={this.enableNewTagTextArea}
                        color="primary"
                        className={styles.addNewTagButton}>Add New</Button>}
              {this.state.newTagTextAreaVisible &&
                <div className={styles.tagInputArea}>
                  ...
                  />
                  <div className={styles.addNewTagConfirm} onClick={this.onAddTag} />
                </div>}
            </div>
          </DialogContent>
          <DialogActions>
