...
const CHANGE_WITH_DEBOUNCE = TranscriptRedux.transcriptNamespace + '_CHANGE_WITH_DEBOUNCE';
...
function* watchContentUndo () {
  yield takeEvery(
    (action) => action.type === TranscriptRedux.UNDO,
    function* (action) {
...
function* watchContentRedo () {
  yield takeEvery(
    (action) => action.type === TranscriptRedux.REDO,
    function* (action) {
...
function* watchContentReset () {
  yield takeEvery(
    (action) => action.type === TranscriptRedux.RESET,
    function* (action) {
...
function* watchContentChange () {
  yield takeEvery(
    (action) => action.type === CHANGE_WITH_DEBOUNCE,
    function* (action) {
      yield put({
